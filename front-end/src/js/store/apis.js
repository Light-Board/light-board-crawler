...
/**
 * @description 모든 키워드 추천수 1등 가져오기
 * @returns res object
 */
 export const getAllKeywordRank = async () => {
  const res = await fetch(`${server_name}/api/keyword/rank`, {
    method: "GET", // *GET, POST, PUT, DELETE, etc.
    mode: "cors", // no-cors, cors, *same-origin
    cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
    credentials: "same-origin", // include, *same-origin, omit
...
// /**
//  * @description 타겟 잡 ID, 내가 추천 누른 Job 리스트 가져오기
//  * @returns res object
//  */
//  export const getMyRecommendList = async (jobId) => {
//   const res = await fetch(
//     `${server_name}/api/job/${jobId}`,
//     {
//       method: "PUT", // *GET, POST, PUT, DELETE, etc.
//       mode: "cors", // no-cors, cors, *same-origin
//       cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
